...
FROM navystack/ngx_mod:${NGINX_VERSION} as nginx-moduler-gnuboard-downloader
...
COPY --from=nginx-moduler-gnuboard-downloader /usr/lib/nginx/modules/*.so /usr/lib/nginx/modules/
COPY --from=nginx-moduler-gnuboard-downloader --chown=www-data:www-data /usr/src/gnuboard /var/www/html
